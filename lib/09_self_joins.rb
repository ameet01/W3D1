...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    ...
    JOIN routes as end_routes
      ON start_routes.company = end_routes.company AND start_routes.num = end_routes.num
    WHERE
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      stopa.name, end_routes.company, end_routes.num
    FROM
      routes AS start_routes
    JOIN
    ...
    JOIN
      stops AS stopa
      ON end_routes.stop_id = stopa.id
    WHERE
      start_routes.stop_id = (
        SELECT
          stops.id
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      firstbus.num, firstbus.company, transfer.name, secondbus.num, secondbus.company
    FROM
      routes firstbus
    JOIN
      routes AS to_transfer
      ON firstbus.company = to_transfer.company AND firstbus.num = to_transfer.num
    JOIN
      stops AS transfer
    ...
    JOIN
      routes AS secondbus
      ON from_transfer.company = secondbus.company
      AND from_transfer.num = secondbus.num
    WHERE
      firstbus.stop_id = (
        SELECT
          id
